...
const FilterWithStartYear = () => {
  const [filteredWithStartYearCompanies, setFilteredWithStartYearCompanies] =
    useState([]);
  ...
  useEffect(() => {
    fetch(
      "https://peaceful-plains-79276.herokuapp.com/filteredWithStartYear?search=" +
        filterWithStartYear
    )
      .then((res) => res.json())
    ...
    <div>
      <div className="row">
        <div class="input-group mb-3 w-50 text-center container-fluid mt-4 col-lg-10">
          <span class="input-group-text">
            <h4>
              Filter with <span className="text-primary">StartYear</span>
            </h4>
          </span>
          <input
            type="text"
            placeholder="Input a StartYear name"
            onChange={handleFilterWithStartYear}
            class="fs-5 form-control"
            aria-label="Amount (to the nearest dollar)"
          />
          <span class="input-group-text">
            {filteredWithStartYearCompanies.length >= 0 &&
              filteredWithStartYearCompanies.length < 100 && (
      ...
      <div className="container g-3">
        <div className="row row-cols-lg-5">
          {filteredWithStartYearCompanies.map((company) => (
