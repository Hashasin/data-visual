...
const FilterWithEndYear = () => {
  const [filteredWithEndYearCompanies, setFilteredWithEndYearCompanies] =
    useState([]);
  ...
  useEffect(() => {
    fetch(
      "https://peaceful-plains-79276.herokuapp.com/filteredWithEndYear?search=" +
        filterWithEndYear
    )
      .then((res) => res.json())
  ...
  return (
    <div>
      <div className="row">
        <div class="input-group mb-3 w-50 text-center container-fluid mt-4 col-lg-10">
          <span class="input-group-text">
            <h4>
              Filter with <span className="text-primary"> End Year</span>
            </h4>
          </span>
          <input
            type="text"
            placeholder="Input an End Year"
            onChange={handleFilterWithEndYear}
            class="fs-5 form-control"
          />
          <span class="input-group-text">
            {filteredWithEndYearCompanies.length >= 0 &&
              filteredWithEndYearCompanies.length < 100 && (
      ...
      <div className="container g-3">
        {filteredWithEndYearCompanies.length > 0 &&
          filteredWithEndYearCompanies.length < 100 && (
            <div className="filterByEndYear">
              <h3 className="text-center mb-3 table-headline">
                Filtered by End Year:{" "}
              </h3>
