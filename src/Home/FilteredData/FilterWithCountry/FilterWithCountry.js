...
const FilterWithCountry = () => {
  const [filteredWithCountryCompanies, setFilteredWithCountryCompanies] =
    useState([]);
  ...
  useEffect(() => {
    fetch(
      "https://peaceful-plains-79276.herokuapp.com/filteredWithCountry?search=" +
        filterWithCountry
    )
      .then((res) => res.json())
      ...
      <div className="row">
        <div class="input-group mb-3 w-50 text-center container-fluid mt-4 col-lg-10">
          <span class="input-group-text">
            <h4>
              Filter with <span className="text-primary">Country</span>
            </h4>
          </span>
          <input
            type="text"
            placeholder="Input a Country name"
            onChangeCapture={handleFilterWithCountry}
            class="fs-5 form-control"
            aria-label="Amount (to the nearest dollar)"
          />
          <span class="input-group-text">
            {filteredWithCountryCompanies.length >= 0 &&
              filteredWithCountryCompanies.length < 100 && (
